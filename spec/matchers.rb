...
RSpec::Matchers.define :pass_hastag_check do |properties|
  match do |path|
    $logger.debug path
    visit path
    if current_path =~ %r{\/$} || current_path =~ /html$/
  ...
  failure_message do |path|
    "Expected that #{path} would pass the spellcheck." \
      " However, it contains:\n  #{@res} \n in its text: \n#{@text}"
  end
end
...
RSpec::Matchers.define :pass_spellcheck do |properties|
  match do |path|
    ...
    if current_path =~ %r{\/$} || current_path =~ /html$/
      ...
      stdin, stdout, _stderr, _wait_thr = \
        Open3.popen3('hunspell -d en_US -p spec/hunspell_en_US -l')
      @text = page.text
      stdin.print @text
      stdin.close
      @res = stdout.read
...
RSpec::Matchers.define :pass_existence_check do |properties|
  match do |path|
    ...
    @uri = Addressable::URI.parse(Capybara.app_host) +
